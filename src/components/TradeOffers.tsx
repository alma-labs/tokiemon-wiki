...
import {
  ...
  isBaseChain,
  WRONG_CHAIN_ERROR,
  USERNAME_REGISTRY_CONTRACTS,
} from "../config/contracts";
import { MARKETPLACE_ABI, ITEM_ABI, USERNAME_REGISTRY_ABI } from "../config/abis";
        ...
        return (
          ...
          >
            <img
              src={
                itemInfo?.image ||
                `https://raw.githubusercontent.com/alma-labs/tokiemon-lists/main/assets/items/${id.toString()}.png`
              }
  ...
  if (!tradeOffersData?.[0].length) {
    return (
      <div className="space-y-3">
        ...
        <div className="text-center py-12 text-slate-400">No active trade offers</div>
      </div>
    );
  }
    ...
    <div className="space-y-3">
      ...
      {(isAccepting || isCancelling || isConfirming) && (
        <div className="bg-[#1da1f2]/10 border border-[#1da1f2]/20 text-[#1da1f2] p-3 rounded-lg mb-4 flex items-center gap-2">
          ...
          <div className="flex-1">{isConfirming ? "Transaction pending..." : "Confirm in wallet..."}</div>
          {txHash && (
            <a
              href={getExplorerLink(txHash)}
              target="_blank"
              ...
              {offer.offerer === address ? (
                ...
                <div className="relative group">
                  {!canAcceptOffer(offer) && (
                    <div
                      className="absolute bottom-full mb-2 right-0 w-64 p-2 bg-slate-800 rounded-lg text-xs text-slate-300 
      ...
      {filteredOffers.length === 0 && (
        <div className="text-center py-12 text-slate-400">
          {activeTab === "forYou" ? "No offers for you" : "No active trade offers"}
        </div>
      )}
