...
export function CreateTradeOffer({ onClose }: { onClose: () => void }) {
  ...
  const filteredOwnedItems = ownedItems.filter(item => 
    item.name.toLowerCase().includes(searchOffer.toLowerCase()) ||
    item.id.includes(searchOffer)
  )

  const filteredWantItems = Object.values(itemsInfo).filter(item => 
    item.name.toLowerCase().includes(searchWant.toLowerCase()) ||
    item.id.includes(searchWant)
  )
          ...
          {isSuccess ? (
            ...
            <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {/* Offer Items */}
                <div>
                  <div className="text-xs font-medium mb-3 text-slate-400 uppercase tracking-wider">You're Offering:</div>
              ...
              {(showOfferSelector || showWantSelector) && (
                <div className="absolute inset-x-4 top-1/2 -translate-y-1/2 bg-[#1a2432] border border-[#2a3844] 
                  ...
                  {showOfferSelector && ownedItems.length === 0 ? (
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {(showOfferSelector ? filteredOwnedItems : filteredWantItems).map((item) => (
                        <button
                          key={item.id}
                          onClick={() => showOfferSelector ? selectOfferItem(item as ItemWithBalance) : selectWantItem(item)}
